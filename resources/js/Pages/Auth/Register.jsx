          ...
          <React.Fragment key={n}>
            <div
              className={cn(
                ...
                step === n &&
                  'border-[#1e3a5f] bg-white text-[#1e3a5f] shadow-md shadow-[#1e3a5f]/15',                step < n && 'border-slate-200 bg-white text-slate-400',
              )}
            >
...
export default function Register() {
  ...
  const [learnerTab, setLearnerTab] = React.useState('0');
  ...
  const addChild = () => {
    const nextIndex = data.children.length;
    setData('children', [...data.children, emptyChild()]);
    setLearnerTab(String(nextIndex));
  };

  const removeChild = (index) => {
    if (data.children.length <= 1) return;
    const filtered = data.children.filter((_, i) => i !== index);
    setData('children', filtered);
    const cur = parseInt(learnerTab, 10);
    let nextIndex = cur;
    if (index === cur) {
      nextIndex = Math.min(cur, filtered.length - 1);
    } else if (index < cur) {
      nextIndex = cur - 1;
    }
    setLearnerTab(String(Math.max(0, nextIndex)));
  };

  const updateChild = (index, patch) => {
    const copy = [...data.children];
    copy[index] = { ...copy[index], ...patch };
    setData('children', copy);
  };
 origin/fix
  ...
  React.useEffect(() => {
    ...
    setData(
      'children',
      data.children.map((c) => {
        if (!c.same_address_as_guardian) return c;
        return {
          ...c,
          house_street_sitio: data.guardian.house_street_sitio ?? '',
          barangay: data.guardian.barangay ?? '',
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only re-sync when guardian address parts change
  }, [
    step,
    data.guardian.house_street_sitio,
    data.guardian.barangay,
    data.guardian.municipality_city,
    data.guardian.province,
  ]);

  const validationErrorsSummary = React.useMemo(() => {
    const keys = Object.keys(errors ?? {});
    if (!keys.length) return null;
    return keys.slice(0, 5).map((k) => errors[k]);
  }, [errors]);
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          <Label>Mother tongue</Label>
                          <Input
                            value={data.guardian.mother_tongue ?? ''}
                            className={inputClass}
                            onChange={(e) => updateGuardian({ mother_tongue: e.target.value })}
                          />
                        </div>
                        <div className="space-y-1.5">
                          <Label>IP / ethnicity (optional)</Label>
                        ...
                        <div className="space-y-1.5">
                          <Label>Barangay</Label>
                          <Input
                            value={data.guardian.barangay ?? ''}
                            className={inputClass}
                            onChange={(e) => updateGuardian({ barangay: e.target.value })}
                          />
                        </div>
                        <div className="space-y-1.5">
                          <Label>Municipality / city</Label>
                          <Input
                            value={data.guardian.municipality_city ?? ''}
                            className={inputClass}
                            onChange={(e) =>
                              updateGuardian({ municipality_city: e.target.value })
                            }
                          />
                        </div>
                        <div className="space-y-1.5 col-span-2">
                          <Label>Province</Label>
                          <Input
                            value={data.guardian.province ?? ''}
                            className={inputClass}
                            onChange={(e) => updateGuardian({ province: e.target.value })}
                          />
                        </div>
                      </div>
                      ...
                      <Tabs value={learnerTab} onValueChange={setLearnerTab} className="w-full">
                              ...
                              {data.children.map((child, index) => {
                                ...
                                return (
                                  <TabsTrigger
                                    key={index}
                                    value={String(index)}
                                    className="shrink-0 min-w-[6.5rem] max-w-[12rem] truncate rounded-lg px-3 py-2 text-xs font-semibold sm:min-w-[7.5rem] sm:text-sm data-[state=active]:bg-[#1e3a5f] data-[state=active]:text-white data-[state=active]:shadow-md"
                                  >
                                    {tabName || `Learner ${index + 1}`}
                                  </TabsTrigger>
                                );
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  variant="outline"
                                  size="icon"
                                  className="h-10 w-10 shrink-0 self-center rounded-xl border-dashed border-slate-300 text-[#1e3a5f] hover:border-[#1e3a5f] hover:bg-[#1e3a5f]/5"
                                  onClick={addChild}
                                  aria-label="Add another learner"
                                >
                                  <Plus className="h-4 w-4" />
                        ...
                        {data.children.map((child, index) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-2 border-b border-slate-100 pb-3">
                              <div>
                                <p className="text-sm font-bold text-[#1e3a5f]">Learner {index + 1}</p>
                                <p className="text-xs text-slate-500">Demographics and address for this learner.</p>
                              </div>
                              {data.children.length > 1 && (
                                <Button
                              ...
                              <div className="space-y-1.5 col-span-2 rounded-lg border border-slate-100 bg-slate-50/80 p-3">
                                <div className="flex items-start gap-3">
                                  <Checkbox
                                    ...
                                    className="mt-0.5 border-slate-300 data-[state=checked]:border-[#1e3a5f] data-[state=checked]:bg-[#1e3a5f]"
                                  />
                                  <div>
                                    <Label
                                      htmlFor={`same-addr-${index}`}
